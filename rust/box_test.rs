...
fn boxed_origin()
    -> Box<Point>{
    //allocates the point on the heap and returns the ptr to it.
...
fn main() {
    ...
    println!("Point occupies {} bytes in a stack", mem::size_of_val(&point));
    println!("Rectangle occupies {} bytes in a stack", mem::size_of_val(&rectangle));
    ...
    println!("Boxed box occupies {} bytes on the stack", mem::size_of_val(&box_in_box));
